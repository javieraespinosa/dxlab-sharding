...
The objective of this exercise is to illustrate the concept of [sharding][1], a database partitioning technique for storing large data collections across multiple database servers. For this purpose, you will work with [MongoDB][2], a document oriented database management system supporting different sharding strategies.
...
MongoDB supports sharding via a _sharded cluster_. A sharded cluster is composed of the following components (see figure below): 
...
Keep in mind that cluster components are _mongo servers_.

In MongoDB, sharding is enabled on _per collection_ basis. When enabled, MongoDB distributes the documents of the collection across the shards (i.e. mongo servers) of a cluster (see example below).

![sharded-collection]

## Preparing a Sharded Cluster  
For simplicity, you will start working with a _partially configured cluster_ composed of:
...
As shown in the figure below, only the **query router** and **config server** are configured as part of a cluster. Let's complete the cluster by adding a shard server. ![docker-cluster-img]Enter the cluster environment:
...
```Finally, run some queries over **cities**:
...
Recall that in MongoDB, sharding is enabled _per collection_. When enabled, MongoDB distributes the documents in a collection using the [shard key][11], an attribute that exists in every document in the target collection.

MongoDB uses two kinds of partitioning strategies:+ **Range based partitioning**: data is partitioned into [chunks][12] (intervals ```[min, max]```) determined by the shard key.
...
Independently of the selected partition strategy, when a shard has too many chunks compared to other shards in the cluster, MongoDB automatically redistributes the chunks across shards. This process is called **chunk balancing**. 

Let's trigger chunk balancing by adding more shards to your cluster.
...
> #### Q7. Draw the new configuration of the cluster and label each element (router, config server and shards) with its corresponding port as you defined in the previous tasks.


## Guiding Partitioning Using Tags

MongoDB also supports tagging a _range of shard key values_. Some advantages of using tags are: 

+ Isolation of data on specific shards.
+ Colocation of shards in geographical regions.

In what follows, you will use tags for isolating some cities into specific shards.
...
```bash
docker-compose down
docker rmi -f mongo:3.0 jupyter/base-notebook
```
